// 3D convolution kernel + simple CPU validation
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "3D Convolution: " << width << "x" << height << "x" << depth
              << "  K=" << K << "  r=" << r << std::endl;
    ...
    float *d_N = nullptr, *d_F = nullptr, *d_out = nullptr;
    CHECK_CUDA_ERROR(cudaMalloc(&d_N, size_bytes));
    CHECK_CUDA_ERROR(cudaMalloc(&d_F, kernel_bytes));
    CHECK_CUDA_ERROR(cudaMalloc(&d_out, size_bytes));
    CHECK_CUDA_ERROR(cudaMemcpy(d_N, h_N.data(), size_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA_ERROR(cudaMemcpy(d_F, h_F.data(), kernel_bytes, cudaMemcpyHostToDevice));
    ...
    // GPU computation
    convolution_3D_basic_kernel<<<grid, block>>>(d_N, d_F, d_out, r, width, height, depth);
...
// (removed duplicate trailing content)
